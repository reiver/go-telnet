...
// A DataWriter deals with "escaping" according to the TELNET protocol.
//
...
//
//(DataWriter is targetted toward TELNET 'data', not TELNET 'commands'.)
//
...
//
// DataWriter takes care of all this for you, so you do not have to do it.
...
//
// For example, if the following it written to the DataWriter's Write method:
//
//	[]byte{1, 55, 2, 155, 3, 255, 4, 40, 255, 30, 20}
//
...
// Write writes the TELNET escaped for of the data in 'data' to the wrapped io.Writer.
func (w *DataWriter) Write(data []byte) (n int, err error) {
	var n64 int64
...
func (w *DataWriter) write64(data []byte) (n int64, err error) {
